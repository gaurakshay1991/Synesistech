...
const sections = ['Decision', 'Clauses', 'Regulatory', 'Exposure', 'Graph', 'Brain', 'Law'] as const;
...
export default function MatterDetail() {
  ...
  const [question, setQuestion] = useState('What is genuinely worth raising in this exact document, what can be let go, and is any cited law or circular stale, amended or incomplete?');
  ...
  async function load() {
    ...
    try {
      ...
      if (documentResult.status === 'fulfilled') {
        setDoc(documentResult.value.document);
        setRegulatory(documentResult.value.document?.analysis?.regulatory_impact || null);
  ...
  async function refreshRegulatory() {
    if (!id) return;
    setBusy('regulatory'); setError('');
    try { const data = await SynesisAPI.regulatoryImpact(id); setRegulatory(data.regulatoryImpact); }
    catch (e: any) { setError(e.message || 'Regulatory impact verification failed.'); }
    finally { setBusy(''); }
  }
  ...
  const reg = regulatory || a.regulatory_impact || {};
  ...
  return <Screen><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    {doc ? <>
      <View style={{ gap: 7 }}><Eyebrow>ONE DOCUMENT · CURRENT-LAW · COMPLIANCE IMPACT</Eyebrow><Text style={styles.title}>{doc.title}</Text><Muted>{doc.matter || 'General review'} · {doc.jurisdiction || '—'} · {doc.documentType || 'Document'}</Muted></View>
      <Card style={{ gap: 9 }}><View style={styles.between}><Pill value={a.overall_risk || doc.overallRisk} /><Pill value={reg.overallImpact || 'UNASSESSED'} /></View><Text style={{ color: C.text, fontWeight: '800' }}>{statusLine}</Text><Muted>Stale authority warning: {reg.staleReferenceWarning ? 'YES' : reg.status === 'COMPLETE' ? 'none identified in verified set' : 'not verified'} · Cross-document memory excluded.</Muted></Card>
      ...
      {section === 'Regulatory' ? <>
        <View style={styles.between}><Text style={styles.sectionTitle}>Regulatory + compliance impact</Text><Action title={busy === 'regulatory' ? 'Re-checking…' : 'Re-check now'} secondary onPress={refreshRegulatory} disabled={Boolean(busy)} /></View>
        <Card style={{ gap: 8 }}><View style={styles.between}><Pill value={reg.overallImpact || 'UNASSESSED'} /><Pill value={reg.staleReferenceWarning ? 'STALE AUTHORITY' : reg.status || 'NOT VERIFIED'} /></View><Text style={{ color: C.text, fontWeight: '900' }}>{reg.executiveConclusion || reg.decisionEffect || 'No regulatory-impact conclusion recorded.'}</Text></Card>
        {(reg.citedAuthorities || []).map((item: any, index: number) => <Card key={item.referenceId || index} style={{ gap: 7 }}><View style={styles.between}><Pill value={item.currentStatus || 'UNCLEAR'} /><Muted>{item.effectiveDate || 'Effective date not established'}</Muted></View><Text style={{ color: C.text, fontWeight: '900' }}>{item.documentReference}</Text><Muted>Current instrument: {item.currentInstrument || 'Not identified'}</Muted><Text style={{ color: C.text, lineHeight: 20 }}>{item.amendmentOrSupersession || item.impact || 'No change explanation returned.'}</Text></Card>)}
        {(reg.omittedApplicableAuthorities || []).length ? <><Text style={styles.sectionTitle}>Applicable law omitted from the document</Text>{reg.omittedApplicableAuthorities.map((item: any, index: number) => <Card key={index} style={{ gap: 7 }}><Text style={{ color: C.text, fontWeight: '900' }}>{item.authority || item.currentInstrument || item.title || 'Applicable authority'}</Text><Muted>{item.whyApplicable || item.applicability || item.why}</Muted><Text style={{ color: C.text }}>{item.impact || item.requiredAction}</Text></Card>)}</> : null}
        {(reg.complianceImpacts || []).length ? <><Text style={styles.sectionTitle}>Compliance impact</Text>{reg.complianceImpacts.map((item: any, index: number) => <Card key={index} style={{ gap: 7 }}><View style={styles.between}><Pill value={item.impactLevel || 'Medium'} /><Muted>{item.ownerFunction || 'Owner not determined'}</Muted></View><Text style={{ color: C.text, fontWeight: '900' }}>{item.dimension || 'Compliance'}</Text><Text style={{ color: C.text }}>{item.requirement}</Text><Muted>Deadline: {item.deadline || 'not established'} · Trigger: {item.trigger || 'applicability'}</Muted><Muted>Control change: {item.controlChange || item.mitigation || 'To be determined'}</Muted></Card>)}</> : null}
        <Card style={{ gap: 7 }}><Text style={{ color: C.text, fontWeight: '900' }}>Transition / deadlines</Text><JsonBlock value={reg.transitionAndDeadlines || []}/><Text style={{ color: C.text, fontWeight: '900' }}>Required actions</Text><JsonBlock value={reg.requiredActions || []}/><Text style={{ color: C.text, fontWeight: '900' }}>Approvals / escalations</Text><JsonBlock value={reg.approvalsAndEscalations || []}/>{(reg.currentLawCitations || []).map((source: any, index: number) => <Pressable key={`${source.url}-${index}`} onPress={() => Linking.openURL(source.url)}><Text style={{ color: C.blue, lineHeight: 19 }}>{source.title || source.url}</Text></Pressable>)}</Card>
      </> : null}

      {section === 'Exposure' ? <><View style={styles.between}><Text style={styles.sectionTitle}>Quantifiability</Text><Action title={exposure ? 'Refresh' : 'Refresh exposure'} secondary onPress={quantify} disabled={Boolean(busy)} /></View>{exposureItems.map((x: any, index: number) => <Card key={x.findingId || index} style={{ gap: 8 }}><View style={styles.between}><Pill value={x.riskLevel} /><Muted>{x.quantificationStatus}</Muted></View><Text style={{ color: C.text, fontWeight: '900' }}>{x.exposureLabel || 'Not reliably quantifiable'}</Text><Muted>{x.rationale}</Muted><JsonBlock value={x.directContractualExposure}/><Muted>Confidence {x.confidence ?? '—'}%. Scenario bands are risk-management assumptions, not legal maxima or expected loss.</Muted></Card>)}{exposure?.authorityResearch ? <Card><Eyebrow>STATUTORY / REGULATORY OVERLAY</Eyebrow><Muted>{exposure.authorityResearch.answer || exposure.authorityResearch.error || exposure.authorityResearch.status}</Muted></Card> : null}{!exposureItems.length ? <Card><Muted>No exposure model is recorded. High/Medium severity alone must never be converted into money.</Muted></Card> : null}</> : null}
