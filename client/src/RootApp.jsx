...
import { BrainCircuit, ChartNoAxesCombined, Globe2, Landmark, ShieldCheck, Workflow } from 'lucide-react';
...
export default function RootApp() {
  ...
  return <div className="synesis-root">
    ...
    <div style={{ margin: '0 18px 12px', padding: '10px 14px', display: 'flex', gap: 12, alignItems: 'center', border: '1px solid rgba(37,99,235,.18)', borderRadius: 12, background: 'rgba(239,246,255,.9)', color: '#172554' }}>
      <Globe2 size={19}/>
      <div style={{ flex: 1 }}><strong style={{ display: 'block', fontSize: 13 }}>THEMIS OPEN INTELLIGENCE + CONTROLLED EXECUTION</strong><small style={{ display: 'block', marginTop: 2, lineHeight: 1.4 }}>Uploaded evidence remains the anchor. Synesis converts assessed changes and institutional events into obligations, impact, decisions, remediation actions, approvals, completion evidence and controlled closure.</small></div>
      <ShieldCheck size={19}/>
    </div>
    {mode === 'command' ? <InstitutionalCommand/> : mode === 'regulatory' ? <RegulatoryCommand/> : mode === 'decision' ? <InstitutionalApp/> : <InstitutionalLab/>}
  </div>;
}
