...
export default function RootApp() {
  ...
  return <div className="synesis-root">
    <div className="synesis-solution-switcher" role="navigation" aria-label="Synesis platform navigation">
      <button className={mode === 'home' ? 'active' : ''} onClick={() => setMode('home')}><Building2 size={16} /><span><strong>Synesis Platform</strong><small>Vision, capabilities, moat and solution packs</small></span></button>
    ...
    {mode !== 'home' && <div style={{ margin: '0 18px 12px', padding: '10px 14px', display: 'flex', gap: 12, alignItems: 'center', border: '1px solid rgba(37,99,235,.18)', borderRadius: 12, background: 'rgba(239,246,255,.9)', color: '#172554' }}>
      <Globe2 size={19} />
      <div style={{ flex: 1 }}><strong style={{ display: 'block', fontSize: 13 }}>SYNESIS INSTITUTIONAL INTELLIGENCE LAYER</strong><small style={{ display: 'block', marginTop: 2, lineHeight: 1.4 }}>Uploaded evidence remains the anchor. Themis, Decision Memory, institutional impact mapping and controlled execution are shared platform capabilities. Each solution pack applies them to a different class of institutional work.</small></div>
      <ShieldCheck size={19} />
    </div>}
