...
function protective(ruleId, text, index) {
  ...
  const protections = [
    /shall not use.*(training|analytics|marketing|benchmarking)/,
    /shall not.*outside.*without.*prior written approval/,
    /shall not subcontract.*without.*prior written approval/,
    /remains fully responsible.*subcontract/,
    /within twenty[- ]four.*hours/,
    /bank and regulators.*audit/,
    /liability.*shall not apply.*fraud/,
    /vendor shall indemnify the bank/,
    /bank may terminate for convenience/,
    /shall not use the bank.*without prior written approval/,
    /only authorised representatives.*bind/
  ];
  if (ruleId === 'slow-incident-notification' && /within twenty[- ]four.*hours/.test(context)) return true;
  return protections.some(p => p.test(context)) && /(shall not|prior written|fully responsible|shall indemnify|may terminate)/.test(context);
}
...
function missingClauses(text, type) {
  if (!/vendor|service|outsourc|data processing/i.test(`${type} ${text}`)) return [];
  const checks = [
...
export function buildAnalysisPrompt({ text, baseline, options }) {
  return `Act as LIVE SYNESIS for the Bank. Analyse only the uploaded document; never reuse fixed findings or generic scenarios. Identify actual clauses, missing protections, contradictions and anomalies. Quote exact document evidence. For every issue explain why it is risky for the Bank, how it may materialise, impacts, mitigation, Bank-protective rewrite, owner, risk level and 0-100 score. Generate scenarios only from the document. Mark regulatory matters requiring verification and do not fabricate citations.\n\nContext: ${JSON.stringify(options)}\nBaseline signals to verify, refine or reject: ${JSON.stringify(baseline)}\n\nUPLOADED DOCUMENT\n${text.slice(0, 120000)}`;
}

export async function analyzeDocument({ openai, model, text, options = {} }) {
  const baseline = heuristicAnalyze(text, options);
  if (!openai) return { ...baseline, assumptions_and_limits: [...baseline.assumptions_and_limits, 'OpenAI API is not configured; the deterministic baseline engine produced this result.'] };
  try {
    const response = await openai.responses.create({
      model,
      input: [
        { role: 'system', content: [{ type: 'input_text', text: 'Return precise enterprise legal analysis that strictly follows the supplied JSON schema.' }] },
    ...
    return { ...baseline, ...parsed, engine: 'openai-structured-output' };
  } catch (error) {
    return { ...baseline, engine: 'baseline-fallback', assumptions_and_limits: [...baseline.assumptions_and_limits, `OpenAI analysis failed; baseline used: ${error.message}`] };
  }
}
