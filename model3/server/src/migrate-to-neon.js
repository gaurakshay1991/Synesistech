...
if (!sourceUrl) {
  throw new Error('DATABASE_URL is required for the source database migration.');
}
...
const TABLES = [
  ...
  { name: 'users', key: 'id' },
  { name: 'institutional_state', key: 'organization_id' },
  { name: 'documents', key: 'id' },
  { name: 'analyses', key: 'id' },
...
async function fingerprint(client, table, key) {
  const result = await client.query(`
    SELECT count(*)::bigint AS count,
      md5(COALESCE(string_agg(md5(to_jsonb(t)::text), '' ORDER BY ${q(key)}::text), '')) AS fingerprint
    FROM ${q(table)} t
  `);
...
try {
  ...
  for (const table of TABLES) {
    const [from, to] = await Promise.all([
      fingerprint(sourceClient, table.name, table.key),
      fingerprint(targetClient, table.name, table.key)
    ]);
