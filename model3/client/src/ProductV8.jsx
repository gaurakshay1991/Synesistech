...
import {
  AlertTriangle, ArrowRight, BrainCircuit, Check, ChevronRight, Clipboard, Copy, FilePlus2, FileText,
  Fingerprint, Globe2, LogOut, Network, RefreshCw, Scale, Search, Send, ShieldCheck, Sparkles,
  UploadCloud, X, Zap
} from 'lucide-react';
  ...
  return <div className="v8-shell">
    <aside className="v8-sidebar">
      ...
      <div className="v8-doc-list">
        ...
        {!documents.length && <p className="v8-muted">No uploaded matters. This workbench intentionally excludes demo matters.</p>}
      </div>
      <div className="v8-runtime">
        ...
        <span><ShieldCheck />Demo seed content suppressed</span>
      </div>
    ...
    <main className="v8-main">
      <header className="v8-topbar">
        <div><small>DOCUMENT-DERIVED · LIVE INTELLIGENCE REQUIRED</small><h1>{detail?.title || 'Matter Intelligence Workbench'}</h1></div>
      ...
      {!detail ? <Empty title="Give SYNESIS a real document" text="The workbench will analyse only the uploaded matter, verify current law, quantify only defensible exposure and let you work each clause." action={() => setUploadOpen(true)} actionLabel="Upload first document" /> : <>
        <div className="v8-status-strip">
          ...
          <div><span>Neural analysis</span><strong>{analysis.analysis_details?.live_ai_used ? 'USED' : 'NOT USED'}</strong></div>
          <div><span>Current-law web</span><strong>{analysis.analysis_details?.current_law_web_used ? 'USED' : 'NOT USED'}</strong></div>
          <div><span>Isolation</span><strong>ONE DOCUMENT</strong></div>
        </div>

        <nav className="v8-tabs">
          {[['matter','Clear / raise / let go'],['findings',`Clause review (${analysis.findings?.length || 0})`],['exposure','Exposure'],['graph','Clause memory graph'],['brain','Ask this matter'],['law','Current law'],['work','Actions']].map(([key,label]) => <button key={key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)}>{label}</button>)}
        </nav>

        {tab === 'matter' && <MatterDecision detail={detail} analysis={analysis} showError={showError} />}
        {tab === 'findings' && <FindingWorkbench detail={detail} onMemorySaved={() => openDocument(detail.id)} showError={showError} showSuccess={showSuccess} />}
        {tab === 'exposure' && <ExposureView analysis={analysis} />}
    ...
    {uploadOpen && <UploadModal onClose={() => setUploadOpen(false)} onDone={async doc => { setUploadOpen(false); await refresh(doc.id); showSuccess('Live document analysis completed.'); }} showError={showError} />}
  </div>;
}
...
function Login({ onDone }) {
  ...
  return <div className="v8-auth"><div className="v8-auth-copy"><div className="v8-logo large"><Zap /></div><small>SYNESIS v8</small><h1>Work the matter. Not a demo dashboard.</h1><p>Upload the document, decide what matters, quantify what can be quantified, rewrite the clause and preserve only human-approved memory.</p></div><form className="v8-auth-card" onSubmit={submit}><h2>Sign in</h2><label>Email<input type="email" value={email} onChange={e => setEmail(e.target.value)} required /></label><label>Password<input type="password" value={password} onChange={e => setPassword(e.target.value)} required /></label>{error && <div className="v8-form-error">{error}</div>}<button className="v8-primary" disabled={busy}>{busy ? 'Signing in…' : 'Enter SYNESIS'}</button></form></div>;
}
...
function UploadModal({ onClose, onDone, showError }) {
  const [file, setFile] = useState(null); const [text, setText] = useState(''); const [jurisdiction, setJurisdiction] = useState('India'); const [matter, setMatter] = useState(''); const [objective, setObjective] = useState('Decide what is worth raising, what can be accepted, quantify defensible exposure, mitigate material risk and draft exact protective wording.'); const [busy, setBusy] = useState(false); const [drag, setDrag] = useState(false);
  ...
  return <div className="v8-modal-back"><form className="v8-modal" onSubmit={submit}><header><div><small>LIVE MULTIPASS ANALYSIS</small><h2>Analyse one document independently</h2></div><button type="button" onClick={onClose}><X /></button></header><div className={`v8-drop ${drag ? 'drag' : ''}`} onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={drop}><UploadCloud /><strong>{file ? file.name : 'Drop PDF, DOCX or supported text file'}</strong><input type="file" onChange={e => setFile(e.target.files?.[0] || null)} /></div><div className="v8-or">or paste text</div><textarea value={text} onChange={e => setText(e.target.value)} placeholder="Paste agreement, clause, policy, circular or case material…" /><div className="v8-form-row"><label>Jurisdiction<input value={jurisdiction} onChange={e => setJurisdiction(e.target.value)} /></label><label>Matter / transaction<input value={matter} onChange={e => setMatter(e.target.value)} placeholder="Optional" /></label></div><label>What should SYNESIS optimise for?<textarea className="short" value={objective} onChange={e => setObjective(e.target.value)} /></label><div className="v8-modal-foot"><span><ShieldCheck />If live neural analysis fails, the app will fail visibly instead of returning a template as a completed review.</span><button className="v8-primary" disabled={busy || (!file && text.trim().length < 20)}>{busy ? <><RefreshCw className="spin" /> Analysing document + current law…</> : <><BrainCircuit /> Analyse now</>}</button></div></form></div>;
}

function MatterDecision({ detail, analysis, showError }) {
  const [pack, setPack] = useState(null); const [busy, setBusy] = useState(false);
  async function run() { setBusy(true); try { const data = await api(`/documents/${detail.id}/decision-pack`, { method:'POST', body:JSON.stringify({ objective:'Can this matter be cleared? Separate must-fix points, worth-raising points, acceptable points and points to let go. Give a senior-lawyer negotiation plan.' }) }); setPack(data.decisionPack); } catch (err) { showError(err); } finally { setBusy(false); } }
  return <div className="v8-grid two">
    <Section title="Current analysis" subtitle={analysis.engine || 'Live matter analysis'}><div className="v8-decision"><Sparkles /><div><strong>{analysis.recommended_decision || 'Run the clearance decision.'}</strong><p>{analysis.executive_position || analysis.document_summary}</p></div></div><button className="v8-primary full" onClick={run} disabled={busy}>{busy ? <><RefreshCw className="spin" /> Re-evaluating current law + matter…</> : <><Scale /> Can this be cleared?</>}</button></Section>
    <Section title="Clearance decision" subtitle="Fresh matter-specific senior review; not seeded"><>{!pack && !busy && <p className="v8-muted">Run the clearance decision to classify every material point into must-fix, negotiate, accept-with-note or let-go.</p>}{busy && <Loader label="Building clearance pack…" />}{pack && <div className="v8-pack"><div className="v8-pack-head"><Risk value={pack.overall_disposition} /><strong>{pack.clearance_recommendation}</strong></div><p>{pack.executive_rationale}</p><DispositionGroup title="Must fix" items={pack.must_fix} tone="critical"/><DispositionGroup title="Raise / negotiate" items={pack.raise_and_negotiate} tone="high"/><DispositionGroup title="Accept with note" items={pack.acceptable_with_note} tone="medium"/><DispositionGroup title="Let go" items={pack.let_go} tone="low"/><h4>Negotiation plan</h4><pre>{typeof pack.negotiation_plan === 'string' ? pack.negotiation_plan : JSON.stringify(pack.negotiation_plan, null, 2)}</pre></div>}</></Section>
  </div>;
}
...
function ActionPack({ pack, onSave }) {
  ...
  return <div className="v8-action-pack"><div className="v8-action-summary"><div><small>Disposition</small><strong>{pack.disposition}</strong></div><div><small>Priority</small><strong>{pack.priority}/5</strong></div><div><small>Confidence</small><strong>{pack.confidence ?? '—'}%</strong></div></div><h4>{pack.headline}</h4><p>{pack.why}</p><div className="v8-grid two"><div><h5>Worth raising?</h5><p>{typeof pack.worth_raising === 'string' ? pack.worth_raising : JSON.stringify(pack.worth_raising)}</p><h5>Risk if accepted</h5><p>{typeof pack.risk_if_accepted === 'string' ? pack.risk_if_accepted : JSON.stringify(pack.risk_if_accepted)}</p><h5>Residual risk</h5><p>{typeof pack.residual_risk === 'string' ? pack.residual_risk : JSON.stringify(pack.residual_risk)}</p></div><div><h5>Quantification</h5><pre>{JSON.stringify(pack.quantification, null, 2)}</pre><h5>Current-law relevance</h5><p>{typeof pack.current_law_relevance === 'string' ? pack.current_law_relevance : JSON.stringify(pack.current_law_relevance)}</p></div></div><div className="v8-rewrite"><div className="v8-rewrite-head"><h5>Exact rewrite</h5><button onClick={() => copy(rewrite.preferred_text)}><Copy/> Copy preferred</button></div><label>Current clause<textarea readOnly value={rewrite.current_clause || ''}/></label><label>Preferred wording<textarea value={rewrite.preferred_text || ''} readOnly/></label><label>Fallback wording<textarea value={rewrite.fallback_text || ''} readOnly/></label><p>{rewrite.drafting_rationale}</p></div><div className="v8-grid two"><div><h5>Mitigation strategy</h5><pre>{JSON.stringify(pack.mitigation_strategy, null, 2)}</pre></div><div><h5>Negotiation strategy</h5><pre>{JSON.stringify(pack.negotiation_strategy, null, 2)}</pre></div></div>{pack.currentLawCitations?.length > 0 && <div className="v8-citations"><h5>Current-law sources</h5>{pack.currentLawCitations.map((c,i) => <a href={c.url} target="_blank" rel="noreferrer" key={`${c.url}-${i}`}><Globe2/><span>{c.title || c.url}</span><ChevronRight/></a>)}</div>}<button className="v8-memory" onClick={onSave}><Fingerprint/> Save this validated position to clause memory</button></div>;
}

function ExposureView({ analysis }) {
  const model = analysis.exposure_model || {};
  const items = model.exposures || model.findings || [];
  return <Section title="Exposure: what can actually be quantified?" subtitle="Severity is not converted into money unless the document or verified authority provides a monetary basis"><div className="v8-exposure-list">{items.map((item,index) => <div className="v8-exposure" key={item.findingId || index}><Risk value={item.riskLevel || item.risk}/><div><strong>{item.category || item.issue}</strong><p>{item.issue}</p><small>{item.quantificationStatus}</small></div><div className="v8-exposure-value"><strong>{item.exposureLabel || item.financialExposure || 'Not reliably quantifiable'}</strong><p>{item.rationale}</p><small>Confidence {item.confidence ?? '—'}%</small></div></div>)}</div>{!items.length && <Empty title="No exposure model recorded" text="This matter needs a fresh analysis before financial exposure can be assessed."/>}</Section>;
}
