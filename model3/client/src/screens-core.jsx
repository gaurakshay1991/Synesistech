...
function MetricGrid({ metrics }) {
  const items = [
    ...
    ['Regulatory mappings', metrics.regulatoryUpdatesOpen ?? 0, Globe2, 'Awaiting verification or impact closure'],
    ['Clause memory', `${metrics.clauseMemoryCoverage ?? 0}%`, Fingerprint, 'Validated archetype and feedback coverage'],
    ['Governance readiness', `${metrics.governanceReadiness ?? 0}%`, BrainCircuit, 'AI, ESG, privacy and model-risk controls'],
    ['Scenario portfolio', metrics.simulationCount ?? 0, GitBranch, 'Institutional and clause-level stress tests'],
    ['Avg. cycle time', `${metrics.averageCycleDays}d`, Clock3, 'Intake to governed disposition'],
    ['Prevented exposure', money(metrics.preventedExposure), Target, 'Illustrative protected value']
  ];
...
export function Review({ active, documents, onOpen, request, setActive, setNotice }) {
  ...
  const tabs = [['position', 'Executive position'], ['issues', `Issues (${a.findings?.length || 0})`], ['trace', 'Neuro-symbolic trace'], ['obligations', `Compiled obligations (${a.obligations?.length || 0})`], ['decision', 'Decision path'], ['impact', 'Impact map'], ['challenge', 'Senior challenge'], ['provenance', 'Provenance']];
  return <><div className="review-head"><div><span className="eyebrow">ACTIVE MATTER</span><h2>{active.title}</h2><p>{active.matter} · {active.documentType} · {active.jurisdiction}</p></div><div className="review-score"><RiskBadge value={a.overall_risk} /><strong>{a.overall_score}<small>/100</small></strong><select value={active.status} onChange={e => statusChange(e.target.value)}>{['AI Review Complete', 'In Legal Review', 'In Compliance Review', 'Escalated', 'Final Approved', 'Rejected', 'Closed'].map(s => <option key={s}>{s}</option>)}</select></div></div><div className="tabbar">{tabs.map(([key, label]) => <button key={key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)}>{label}</button>)}</div>
    ...
    {tab === 'trace' && <><div className="trace-score-grid"><article><small>Symbolic signal</small><strong>{a.neuro_symbolic?.symbolic_signal_score ?? '—'}</strong><span>Configured legal rules</span></article><article><small>Neural signal</small><strong>{a.neuro_symbolic?.neural_signal_score ?? '—'}</strong><span>Context and cross-clause interpretation</span></article><article><small>Reconciled score</small><strong>{a.neuro_symbolic?.reconciled_score ?? a.overall_score ?? '—'}</strong><span>Challenge-adjusted decision signal</span></article><article><small>Clause fingerprint</small><b>{a.analysis_details?.clause_fingerprint || 'Not recorded'}</b><span>Memory and equivalence key</span></article></div><div className="two-col wide-left"><Panel title="Reasoning trace" subtitle={a.neuro_symbolic?.architecture || 'Neuro-symbolic architecture'}><div className="reasoning-trace">{(a.reasoning_trace || []).map((item,index) => <div key={item.step || index}><span>{index + 1}</span><div><strong>{item.type}: {item.proposition}</strong><p>{item.evidence}</p></div><RiskBadge value={typeof item.result === 'string' ? item.result : String(item.result ?? '')} /></div>)}</div></Panel><Panel title="Reconciliation and limitations" subtitle="Explainability before reliance"><p className="long-copy">{a.neuro_symbolic?.reconciliation || 'No reconciliation narrative recorded.'}</p><KeyValue label="Rules evaluated" value={a.neuro_symbolic?.symbolic?.metadata?.rulesEvaluated} /><KeyValue label="Rules fired" value={a.neuro_symbolic?.symbolic?.rules_fired?.length || 0} /><KeyValue label="Positive controls" value={a.neuro_symbolic?.symbolic?.positive_controls?.length || 0} /><KeyValue label="Litigation indicator" value={`${a.litigation_risk?.probability ?? '—'}% probability / ${a.litigation_risk?.confidence ?? '—'}% confidence`} /></Panel></div><div className="two-col"><Panel title="Clause-memory candidates" subtitle="Candidates do not become institutional precedent without human validation"><div className="candidate-list">{(a.clause_memory_candidates || []).map((item,index) => <div key={index}><Fingerprint /><span><strong>{item.category || 'Clause candidate'}</strong><small>{item.proposedLesson || item.lesson || item.status}</small></span></div>)}</div></Panel><Panel title="Controlled rewrite candidates" subtitle="Draft language—authorised review required"><div className="rewrite-list">{(a.rewrite_candidates || []).map((item,index) => <div key={index}><strong>{item.issue}</strong><p>{item.proposedText}</p><Status value={item.status} /></div>)}</div></Panel></div></>}
