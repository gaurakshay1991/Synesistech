...
function fallbackAnswer(analysis, question) {
  ...
  const answer = corpus
    .map(item => ({
      item,
      score: terms.reduce((sum, term) => sum + Number(JSON.stringify(item).toLowerCase().includes(term)), 0)
    }))
...
gateway.get('/api/health', (req, res) => {
  res.json({
    ...
    privateWorkspace: config.databaseUrl ? 'configured' : 'not-configured',
...
gateway.post('/api/public/institutional/mandate', publicLimiter, (req, res) => {
  try {
    const text = sourceText(req);
    const portfolio = req.body?.portfolio && typeof req.body.portfolio === 'object'
      ? req.body.portfolio
      : null;
...
gateway.post('/api/public/analyze', publicLimiter, async (req, res) => {
  const text = String(req.body?.text || '').replace(/\u0000/g, '').trim();
  if (text.length < 20) {
    return res.status(400).json({ error: 'Upload or paste enough readable document text.' });
  }
  if (text.length > 180000) {
    return res.status(413).json({ error: 'Document exceeds the 180,000-character public review limit.' });
  }
  ...
  const options = {
    ...
    institutionFunction: cleanString(
      req.body?.institutionFunction,
      req.body?.department || 'Enterprise / Institution',
      120
    ),
    workType: cleanString(req.body?.workType, 'Review this document', 120),
    analysisObjective: cleanString(
      req.body?.analysisObjective,
      'Determine what the evidence means, what is affected, what decision is required and what controlled action should follow.',
      1000
    ),
...
gateway.post('/api/public/ask', publicLimiter, async (req, res) => {
  ...
  if (question.length < 3) {
    return res.status(400).json({ error: 'Enter a question about the active document.' });
  }
