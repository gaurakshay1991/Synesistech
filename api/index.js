...
function fallbackAnswer(analysis, question) {
  ...
  const corpus = [
    ...
    ...(Array.isArray(analysis.decision_intelligence?.decision_questions) ? analysis.decision_intelligence.decision_questions.map(item => ({ question: item })) : [])
  ];
  const answer = corpus
...
gateway.get('/api/health', (req, res) => {
  res.json({
    ...
    analysis: 'live-multipass-institutional-decision-intelligence',
...
gateway.post('/api/public/analyze', publicLimiter, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'LIVE SYNESIS could not analyse this document.', detail: String(error.message || '').slice(0, 240) });
  }
});
...
gateway.post('/api/public/ask', publicLimiter, async (req, res) => {
  ...
  try {
    const response = await openai.responses.create({
      ...
      input: `You are LIVE SYNESIS, an institutional decision intelligence and execution assistant. Answer only from the supplied active-document analysis and its decision model. Distinguish evidence, inference and uncertainty. Do not invent clauses, law, facts or citations. Do not reduce the answer to legal risk alone; address operational, governance, capital, stakeholder, control and execution consequences when the analysis supports them. Refer to the user organisation as the institution unless the document identifies a more precise role.\n\nQUESTION\n${question}\n\nACTIVE ANALYSIS\n${JSON.stringify(analysis).slice(0, 120000)}`
    });
    res.set('Cache-Control', 'no-store');
